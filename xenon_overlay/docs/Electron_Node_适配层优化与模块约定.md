...
- 原生窗口调用产生的几何变化随同步回复交付，main 在方法返回前发布
  `move/resize`。同一次原生调用内部的尺寸校正合并为最终变化，覆盖显式
  配对的其他窗口；不同调用之间不合并。没有新增应用名称或坐标特判。
- 原生用户拖动仍走异步通知。单调递增的窗口版本防止晚到的几何快照覆盖
  新状态，但不丢弃先前实际发生的拖动事件；单纯查询不消费事件。
- main 先更新同批窗口缓存，再调用监听器。监听器中再次设置尺寸或销毁
  窗口时，外层调用不回写旧值。版本和事务字段仅供内部桥接使用，不出现在
  Electron 的 `getBounds()` 等公开返回值中。
- TH 游戏弹窗暴露了该顺序缺陷：设置最小尺寸及普通尺寸的旧 `resize`
  在 `show()` 后才交付，导致应用取消了临时离屏显示后的坐标恢复。修复位于
  通用窗口适配层，未修改 TH 或 PLE 源码，也不改变前台激活策略。
- 正式浏览器构建通过，JS 回归 583 项通过、2 项依赖外部包的用例跳过，
  原生 IPC 回归 209/209 通过。新增 8 项宿主事务用例完成对象编译，未运行
  完整 Chromium `unit_tests`。实际窗口同步事件、离屏恢复、回调重入、
  无重复通知等 9 项检查通过；游戏弹窗恢复到屏幕内，用户确认“正常了”。

## 2026-09-18 显式父子窗口配对

- `IpcMainConfig.parent_window_pairing_urls` 由应用接入配置声明需要与父窗
  配对的控制页面。它是宿主扩展，不改变 Electron `parent` 的标准含义。
  匹配规范化后的完整 URL，忽略 query/fragment，按应用容器隔离，不按
  应用名称、页面标题、透明度或窗口尺寸推断新增配对关系。
- 配对只连接同一容器中仍存活的窗口，双向同步位置、尺寸及既有显示状态。
  同组多个控制窗及配对链一起更新；普通 owner 关系不加入该组。同步使用
  Views 的平台无关窗口接口与边界变化通知，不增加轮询或平台消息特判。
- 仅主框架提交后更新配对 URL，取消导航保留旧文档规则，错误页提交清空。
  `setParentWindow` 后重新核对声明；首次配对先继承父窗现有显示状态，再
  同步几何。移除父窗会停止配对。
  尚未迁移的透明覆盖窗保留原有兼容行为，本轮不扩大该自动匹配范围。
- PLE 接入配置声明 `clipper.html` 与 `gifClipper.html`，无需修改 PLE 源码。
  原来 GIF 的透明控制窗命中了兼容同步，视频截取的不透明控制窗未命中。
  两者的原生 helper 都尝试在 Utility 为 Browser HWND 安装 WndProc，不能
  跨进程完成这种子类化；显式配对在 Browser 中直接维护窗口关系。
- 最终生产构建及增量构建通过，原生 IPC 回归 209/209 通过（不含真实应用
  smoke）。新增 URL 匹配和 8 项窗口组/导航回归用例完成对象编译，未执行
  完整 Chromium `unit_tests`。9222 确认 GPU 保持开启、TH 页面加载正常；
  用户实机确认视频截取拖动恢复正常。PLE 源码工作区仍干净。

## 2026-09-18 Windows 原生子窗口合成

- 托管 BrowserWindow 在 Windows 保留用于原生 GDI 子窗口的重定向位图，
  Web 页面继续使用 GPU 合成；普通浏览器窗口保持 Chromium 默认设置。
- `RenderingWindowManager` 只对显式启用的父窗口处理：按真实 Z 序，将可见、
  不透明原生子窗口的实际区域从 GPU 子窗口中扣除。不按应用名、模块名或
  窗口类名选择；分层和透明子窗口不作不透明遮挡处理。
- 使用跨进程 WinEvent 合并更新，处理移动、尺寸、显隐、销毁及 GPU 窗口
  重建，不使用定时轮询。停用或 compositor 注销时恢复 GPU 原区域；窗口
  的启用状态保留到显式停用或真实销毁，以覆盖 compositor 重建。
- Win32 实现、头文件引用和测试受 `is_win` / `BUILDFLAG(IS_WIN)` 限定。
  公共窗口选项只传递平台无关布尔值，其他平台沿用原有合成路径。
  该修复不实现 `app.disableHardwareAcceleration()`，也不改变双窗拖动关系。
- 本轮 Chromium 扩展统一受 `BUILDFLAG(ENABLE_XENON_SERVICE)` 控制。
  GN 参数 `enable_xenon_service=false` 时不声明新增 Blink/窗口接口，不编译
  裁剪状态、事件监听和专用测试，窗口样式计算与原生管理路径恢复原逻辑。
  buildflags 头文件生成依赖仍保留，不因此链接 Xenon 服务实现。
- 开关关闭的独立 GN 配置生成成功；manager、HWND 样式工具与 Windows
  desktop host 三个生产文件均完成宏值为 0 的实际对象编译。Blink/Views
  公共头文件的 on/off 语法与接口可见性检查通过；本次未全量链接关闭版浏览器。
- Windows 真实 HWND 回归 10/10 通过，覆盖复杂区域、上下层级切换、移动与
  显隐、GPU 重建、晚到销毁事件和原区域恢复。跨平台结论目前来自源码与
  GN 条件检查，未执行 Linux/macOS 编译。
- 最终生产构建与增量构建通过，原生 IPC 回归 209/209 通过（不含真实应用
  smoke）；窗口裁剪 10/10 使用最终构建产物复验通过。以正常启动参数连接
  9222，GPU 合成与光栅化均启用，未使用全局软件合成或重定向位图诊断开关。
  PLE 主播放错误码为 0；截取窗口保留 `software_render`，运行时 GPU 区域
  正确扣除原生预览区域，用户确认正式版本上方可见视频画面。

## 2026-09-18 多窗口原生状态与资源加载

- Renderer 的 native 宿主按 `{container_id, Document endpoint}` 懒启动。
  同页重复 `require` 复用原生导出和 DLL，其他页面与应用 main 使用独立进程。
  仅增加 JS/V8 context 不能隔离 DLL 静态变量；共享应用级宿主会使多窗口的
  初始化互相覆盖，甚至等待同一后台线程退出而卡住。
- 同步调用、异步调用、Promise 续接及原生回调归属同一文档。文档销毁和应用
  退出关闭所属连接；崩溃后的旧文档不能隐式重启 native 宿主。新进程不重跑
  应用 main。代价是每个实际使用 native 模块的文档增加一个 Utility 进程。
- `loadURL/loadFile` 仅按显式 renderer 映射替换 URL，保留相对页面、查询和
  fragment；未匹配文件不得静默降级为入口 `index.html`。产品资源布局的别名
  在接入配置声明，通用窗口实现不判断业务页面名称。
- 映射成 WebUI 的本地文档由 Browser 核对真实宿主与最终路径后，按 Document
  授予本地资源显示能力。原生文件 URL loader 也检查宿主与请求 origin。
  图片 CSP 允许 `file:` 不等于授予读取权限，普通标签和远端导航不获得该能力。
  `about:blank/srcdoc` 按 Blink 规则继承创建者 origin 的能力。普通文件委托
  Chromium 标准 file factory；ASAR 使用相同的跨源响应规则，展示响应为
  `opaque`，跨源 CORS 读取被拒绝，避免展示授权意外变成 canvas 像素读取授权。
- 窗口几何缓存与通知状态分开；程序性调用同步交付当次 `move/resize`，
  查询不得吞掉待交付的原生拖动通知。普通子窗仍保留独立尺寸与位置。
