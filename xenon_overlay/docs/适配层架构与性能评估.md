...
| 优先级 | 问题与证据 | 下一步与约束 |
| --- | --- | --- |
| 高 | ReadStream 现在按请求范围读取，但仍先缓存整个范围；无结束范围时仍读到 EOF。HTTP 使用 `DownloadToString` 后一次交付，上传/响应仍有 Base64。大范围和大响应仍会占用更多内存并延迟第一块数据。 | 第 8 节修复范围放大；真正分块传输仍需测峰值内存、首块延迟，再设计背压、取消和错误顺序，覆盖 ASAR 与提前关闭。 |
...
## 8. ReadStream 范围读取（2026-09-17）

### 修复范围

- main/renderer 的 `ReadStream` 将 `start` 和有限的 inclusive `end` 传入现有
  `read_file` 请求。未指定 `end` 时读到 EOF；普通 `readFile` 请求不带范围，
  保持完整读取语义。JS 不再对已按范围返回的数据做第二次偏移裁剪。
- 原生文件桥从请求起点读取，按结束范围限制读取量，实际读到 EOF 即停止。
  packed ASAR 使用条目的 offset/size 限界，不能读入相邻条目；unpacked
  ASAR 读取实际旁路文件。普通文件不以 stat 长度决定 EOF，避免把长度报告为
  0 的 POSIX 系统文件误判为空。复用现有 Archive 接口，无新增 fd 生命周期。
- 返回仍走 BLOB 通道。保留 UTF-8 跨 chunk 解码、pause/resume、`bytesRead`、
  end/close 顺序和 destroy 后忽略晚返回的行为。

这次只修复“请求短范围却读取和保留整文件”的放大。每次仍先缓存所选范围，
再按 `highWaterMark` 交付；没有结束范围时仍读取 start 至 EOF。
这不是按 highWaterMark 逐次读取磁盘的完整流式实现，destroy 也不会取消已经
提交给 worker 的文件读取。真正分块读取、背压和原生取消仍需单独设计及验证。

### 范围测量

使用第 7 节同一 16 MiB 公开数据模型，请求中间 64 KiB，分别运行 main 和
renderer 的完整生产 bootstrap，`highWaterMark` 分别为 16/64 KiB：

| 指标 | 修复前 | 修复后 |
|---|---:|---:|
| 文件桥请求次数 | 1 | 1 |
| 文件桥返回字节 | 16,777,216 | 65,536 |
| JS 交付字节 | 65,536 | 65,536 |
| 交付块共享的 backing ArrayBuffer | 16 MiB | 64 KiB |
| 返回量 / 请求范围 | 256 | 1 |

四种组合均逐字节一致，`ready/end/close` 各一次。这里的前后对照使用内存
模拟 native transport，不是 OS syscall、Mojo wire、CPU 或进程 RSS 测量。
64 KiB 的范围返回量减少约 99.61%，不能表述为整个应用提速 256 倍。

复现：`node out/readstream-range-20260917/measure-readstream-range.cjs before`
与 `after`。该目录保存两份基线 bootstrap 以及
`readstream-range-before.json`、`readstream-range-after.json`。

### 构建与实机验证

- JS **447/447、零跳过**；Windows 原生 **183/183**（排除
  `XenonRealAppSmokeTest.*`）。新增原生测试使用真实文件和 ASAR，覆盖
  16 MiB 文件中的 64 KiB 范围、多块范围、全范围、EOF、MAX_SAFE_INTEGER、
  非法参数、unpacked 实际长度和旧 Base64/BLOB 全文件读取。
  POSIX 大于 4 GiB 的稀疏文件与 Linux procfs 用例已加入，但本机未执行。
- `chrome` 与 `ipc_main_container_unittests` 完整构建通过；最终资源包内
  main/renderer 脚本与生成文件逐字一致。EOF 探测使用至多 64 KiB 临时缓冲，
  只追加实际读到的字节，避免先扩容结果再发现 EOF。
- 9222 实机 TH/PLE 各 **26/26** 项通过。两端分别检查真实普通文件、
  packed/unpacked ASAR：中间范围的两种 highWaterMark、单字节、end-only、
  start-only、越过 EOF、恰在 EOF、MAX_SAFE_INTEGER，以及两个空文件。
  16 MiB 文件的 64 KiB 范围，真实异步桥返回和交付块 backing 均为
  **65,536 字节**；逐字节、bytesRead 和 ready/end/close 次数一致。
  探针只记录公开夹具请求，不拦截其他文件数据，也不测进程峰值内存。
- TH SDK ready、自动登录和头像加载正常。PLE 播放仓库公开视频，320×180，
  进度 **173 → 1064 ms**，errCode 为 0；关闭测试媒体并通过原启动通路
  重开后无当前媒体、无 HTML 音视频播放。首次探针发现已有媒体而跳过，
  后续确认空闲才开始测试；没有覆盖或关闭非测试媒体。
- 日志快照无 native FATAL、容器断连、N-API 或 app/window 事件失败；
  对照的既有兼容 API、fs.watchFile、child_process.exec 和网络报错仍存在。
  本轮没有执行真实鼠标拖动或菜单视觉验收。

本机证据位于 `out/readstream-range-20260917/`：`js-tests.log`、
`native-tests.log`、`native-results.json`、`build-final.log`、
`shipping-resources.json`、`range-th.json`、`range-ple.json`、
`runtime-evidence.jsonl`、`log-summary.json` 与 `error-comparison.json`。
