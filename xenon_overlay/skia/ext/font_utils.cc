...
skia::GetCustomFontCallback g_custom_font_callback = nullptr;
...
void SetGetCustomFontCallback(GetCustomFontCallback callback) {
  g_custom_font_callback = callback;
}

sk_sp<SkTypeface> MakeTypefaceFromName(const char* name, SkFontStyle style) {
  if (g_custom_font_callback && name) {
    sk_sp<SkTypeface> face = g_custom_font_callback(name, style);
    if (face) {
      return face;
    }
  }
