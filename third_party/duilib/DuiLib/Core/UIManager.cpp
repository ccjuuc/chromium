...
namespace DuiLib {
		...
		case WM_MOUSEMOVE:
			{
				...
				if(bNeedDrag && m_bDragMode && wParam == MK_LBUTTON)
				{
					...
					dragSrcHelper.InitializeFromBitmap(hBitmap, ptDrag, rc, pdobj);
					DWORD dwEffect;
		...
		case WM_TOUCH:
			{
				...
				UINT nTouchCount = LOWORD(wParam);
				...
				if (pGetTouchInputInfo((HTOUCHINPUT)lParam, nTouchCount, pTouchInputs, sizeof(TOUCHINPUT)))
				{
					for (UINT i = 0; i < nTouchCount; ++i)
					{
						...
						WPARAM wWheelParam = MAKEWPARAM(0, nWheelDelta);
						LPARAM lWheelParam = MAKELPARAM(ptTouch.x, ptTouch.y);
	...
	void CPaintManagerUI::SendNotify(CControlUI* pControl, LPCTSTR pstrMessage, WPARAM wParam , LPARAM lParam , bool bAsync )
	{
		TNotifyUI Msg;
	...
	CControlUI* CALLBACK CPaintManagerUI::__FindControlFromCount(CControlUI* , LPVOID pData)
	{
	...
	CControlUI* CALLBACK CPaintManagerUI::__FindControlFromShortcut(CControlUI* pThis, LPVOID pData)
	{
		...
		if( _tcsstr(pThis->GetClass(), _T("LabelUI")) != NULL ) return NULL;
