...
#include <textserv.h>
	...
	private:
		...
		unsigned	fEnableAutoWordSel	:1;
		unsigned	fWordWrap			:1;
		unsigned	fAllowBeep			:1;
		unsigned	fRich				:1;
		unsigned	fSaveSelection		:1;
		unsigned	fInplaceActive		:1;
		unsigned	fTransparent		:1;
		unsigned	fTimer				:1;
		unsigned    fCaptured           :1;
		unsigned    fShowCaret          :1;
		unsigned    fNeedFreshCaret     :1;
...
err:
	...
	{
		LRESULT lResult;
		TxSendMessage(EM_SETEVENTMASK, 0,ENM_DROPFILES|ENM_LINK,
			&lResult);
		return (BOOL)lResult == FALSE;
	}
