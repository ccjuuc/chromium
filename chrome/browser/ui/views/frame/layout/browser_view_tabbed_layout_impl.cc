...
gfx::Size BrowserViewTabbedLayoutImpl::GetMinimumMainAreaSize(
  ...
  width += xenon_sidebar_size.width();
...
BrowserViewTabbedLayoutImpl::CalculateProposedLayout(
  ...
  if (IsParentedTo(views().xenon_sidebar, views().browser_view)) {
    ...
    if (actual_sidebar_width > 0) {
      const bool xenon_sidebar_leading = side_panel_leading;
      ...
      auto side_panel_layout_iter = layout.children.find(views().side_panel);
      if (side_panel_layout_iter != layout.children.end() &&
          !side_panel_layout_iter->second.bounds.IsEmpty()) {
        side_panel_layout_iter->second.bounds.Offset(
